...
#     def __init__(self, dataset_path, json_file_path, split='train', categories=None, transforms=None):
#         """
#         ShapeNet dataset for AbdeinNet.
...
#                 category_id = sample_path.split('/')[0]
#                 model_id = sample_path.split('/')[1]
...
#         # Load multiple views (assuming 24 views as in the original implementation)
#         rendering_path = os.path.join(self.dataset_path, sample['sample_path'], 'rendering')
#         imgs = []
#         for i in range(24):  # 24 views per model
#             img_path = os.path.join(rendering_path, f'{i:02d}.png')
...
#         imgs = torch.stack(imgs, dim=0)  # [n_views, C, H, W]
        
#         # Load voxel ground truth
#         voxel_path = os.path.join(self.dataset_path, sample['sample_path'], 'voxel.npy')
